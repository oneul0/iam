...
| 입력 프롬프트 | 반영된 수정 사항 |
|---|---|
| `SPA를 MPA로 전환하기 위해서는 어떤 작업을 해야 해? 지금은 단일 HTML, CSS, JS야` | 화면별 HTML 분리, 문서 링크 이동, 공통 자산과 페이지 상태 관리 방식을 MPA 기준으로 설계했습니다. |
| `한 단계씩 하자. HTML 줄 테니까 분리해줘` | 단일 HTML을 홈, DM, 스터디, 추천의 네 문서로 분리하고 각 페이지에 필요한 콘텐츠만 남겼습니다. |
| `app.js야` | 하나의 `app.js`를 공통 기능과 페이지별 기능으로 나눠 `common.js`, `home.js`, `dm.js`, `recommendations.js`로 구성했습니다. |
| `style.css` 제공 | `<a>` 기반 메뉴에 맞춰 밑줄 제거와 글꼴 상속을 추가하고, 공통 네비게이션 컨테이너가 축소되지 않도록 보정했습니다. |
| `JS랑 연결이 안 된 것 같아` | HTML이 `html` 폴더에 있고 `js`가 형제 폴더에 있는 구조를 기준으로 스크립트 경로를 `../js/...`로 통일했습니다. |
| `common.js는 모든 페이지에 연결해야 하는 거지?` | 모든 페이지가 `common.js`를 먼저 로드하고, 홈·DM·추천 페이지만 전용 스크립트를 추가로 로드하도록 확정했습니다. |
| `DM에 이미지 넣으려면 어떻게 해야 해? 빵과 고기 부분에 넣고 싶어` | DM 답변 객체의 `image` 속성과 실제 정적 이미지 경로를 이용해 텍스트·이미지 복합 말풍선을 출력하는 방식을 명시했습니다. |
| `MPA 구성 체크리스트에 부합하는지 체크해줘` | 공통 메뉴 일치, 현재 페이지 표시, 링크 이동, 공통 디자인 여부를 검토하고 별도 네비게이션 파일 요구사항을 확인했습니다. |
| `nav를 별도 파일로 저장해서 각 HTML 파일에 삽입하는 방식` | 네비게이션을 `html/includes/nav.html`로 분리하고 모든 페이지에 `navigation-container`만 배치한 뒤 `common.js`가 삽입하도록 변경했습니다. |
| `메뉴를 불러오지 못했습니다` 및 CORS 오류 로그 | `file://` 환경에서 `fetch()`가 차단되는 원인을 확인하고 Live Server 또는 로컬 HTTP 서버 실행을 필수 조건으로 정리했습니다. |
| `Netlify, Vercel 등에서는 정상 작동하는 거지?` | HTTP/HTTPS 정적 호스팅에서의 동작 조건, 디렉터리 유지, `nav.html` 배포 여부, SPA rewrite 제외 조건을 문서화했습니다. |
